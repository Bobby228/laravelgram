            ...
            <div class="flex justify-between items-center mb-6 pb-6 border-b border-gray-400" v-for="user in users">
                ...
                <div>
                    <a @click.prevent="toggleFollowing(user)"
                       :class="['block p-2 text-center w-32 text-sm rounded-3xl', user.is_followed ? 'bg-white text-sky-500 border border-sky-500' : 'bg-sky-500 text-white']" href="#">{{ user.is_followed ? 'Unfollow' : 'Follow' }}</a>
                </div>
            </div>
        </div>
    </div>
